namespace FMS_API.Entities
{
    public class T_OIMMAIN
    {
        public int F_ID { get; set; }
        public string F_MBLNo { get; set; }
        public string? F_RefNo { get; set; }
        public string? F_AgentRefNo { get; set; }
        public string? F_SMBLNo { get; set; }
        public string? F_mCommodity { get; set; }
        public int? F_Shipper { get; set; }
        public string? F_SName { get; set; }
        public int? F_Consignee { get; set; }
        public string? F_CName { get; set; }
        public int? F_Notify { get; set; }
        public string? F_NName { get; set; }
        public int? F_Broker { get; set; }
        public string? F_BName { get; set; }
        public int? F_Agent { get; set; }
        public string? F_AgentName { get; set; }
        public DateTime? F_ETA { get; set; }
        public DateTime? F_ETD { get; set; }
        public string? F_Vessel { get; set; }
        public string? F_Voyage { get; set; }
        public int? F_Carrier { get; set; }
        public string? F_CarrierName { get; set; }
        public int? F_LCode { get; set; }
        public string? F_LoadingPort { get; set; }
        public int? F_DCode { get; set; }
        public string? F_DisCharge { get; set; }
        //public int? F_CYLocation { get; set; }
        //public int? F_CFSLocation { get; set; }
        public DateTime? F_PostDate { get; set; }
        public string? F_LCLFCL { get; set; }
        public int? F_TotalContainer { get; set; }
        //public int? F_ITCarrier { get; set; }
        public string? F_ITNo { get; set; }
        public string? F_ITPlace { get; set; }
        //public string? F_ITClass { get; set; }
        public DateTime? F_ITDate { get; set; }
        public int? F_FCode { get; set; }
        public string? F_FinalDelivery { get; set; }
        public string? F_FinalDest { get; set; }
        public DateTime? F_FETA { get; set; }
        public string? F_PPCC { get; set; }
        public string? F_PaidPlace { get; set; }
        public string? F_SVCCNo { get; set; }
        public string? F_FeederVessel { get; set; }
        public string? F_MoveType { get; set; }
        public string? F_ExpRLS { get; set; }
        public string? F_FileClosed { get; set; }
        public string? F_ClosedBy { get; set; }
        public string? F_Memo { get; set; }
    }
}
